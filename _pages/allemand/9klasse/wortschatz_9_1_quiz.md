...
permalink: /allemand_main/9klasse/wortschatz1_quiz
---
...
function updateFlashcard() {
  currentMemberIndex = getRandomMemberIndex(); // Update currentMemberIndex with a new random index
  const member = flashcardData[currentMemberIndex]; // Get the member with the updated index
  document.getElementById('french-word').innerText = member[varFront];
  clearResults();
}

function checkAnswer() {
  const member = flashcardData[currentMemberIndex]; // Get the current member based on currentMemberIndex
  ...
  if (inputArticle.toLowerCase() === member[varArtikel].toLowerCase()) {
    ...
    resultArticle.innerHTML = `<i class="fas fa-check-circle" style="color: green;"></i> ${inputArticle}`;
  } else {
    resultArticle.style.color = 'red';
    resultArticle.innerHTML = `<i class="fas fa-times-circle" style="color:red;"></i> <s>${inputArticle}</s> <span style="color: #007BFF;">${member[varArtikel]}</span>`;
  }

  if (inputWord === member[varBack]) {
    resultWord.style.color = 'green';
    resultWord.innerHTML = `<i class="fas fa-check-circle" style="color:green;"></i> ${inputWord}`;
  } else {
    resultWord.style.color = 'red';
    resultWord.innerHTML = `<i class="fas fa-times-circle" style="color:red;"></i> <s>${inputWord}</s> <span style="color: #007BFF;">${member[varBack]}</span>`;
  }
}

// Function to get the next member index
function getRandomMemberIndex() {
...
// Move the check and next buttons outside the flashcard container
...
// nextButton.onclick = updateFlashcard; // Call updateFlashcard instead of nextMember
